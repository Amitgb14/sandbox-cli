...
// Sharing lives here rather than in internal/cli because it has three callers
// and one of them is a daemon. An interactive run, a fleet launch and a Studio
// request must arrange the *identical* mount — this is the code that turns
// "--share" into a host path handed to docker, and two copies of that would be
// two answers to a question about the boundary.
//
// What stays with the caller is the announcement: a terminal has somewhere to
// print "sharing X at Y" and an HTTP handler does not.
...
// SharedMount is a resolved --share: the mount string, and the two halves of it
// a caller may want to report.
type SharedMount struct {
	// Mount is the host:container:rw string, ready for RunSpec.ExtraMounts.
	Mount string
	// Host is the directory on this machine, and Target where it lands in the
	// container. Both are derived from Mount and are carried separately so a
	// caller announcing the share does not have to parse it back apart.
	Host   string
	Target string
}
...
func ShareMount(name string) (SharedMount, error) {
	root := config.SharedDir()
	if root == "" {
		return SharedMount{}, fmt.Errorf("--share: cannot determine the config directory (no HOME?)")
	}
	if err := os.MkdirAll(root, 0o700); err != nil {
		return SharedMount{}, fmt.Errorf("creating shared dir %s: %w", root, err)
	}
	...
	if err := RefuseUnsafeHostPath(root); err != nil {
		return SharedMount{}, fmt.Errorf("--share: %w", err)
	}
	...
	if err := RefuseUnsafeHostPath(dir); err != nil {
		return SharedMount{}, fmt.Errorf("--share: %w", err)
	}
	ShareWithSandboxGroup(dir)
	return SharedMount{Mount: dir + ":" + target + ":rw", Host: dir, Target: target}, nil
}
...
func ShareNamespaceDir(root, name string) (hostDir, target string, err error) {
	...
	if err := RefuseUnsafeHostPath(resolved); err != nil {
		return "", "", err
	}
