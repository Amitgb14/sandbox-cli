...
// authorized enforces the optional bearer token. The health route is exempt so a
// client can discover whether the server is up before it has a token to present.
//
// It reads healthPath rather than a literal: the routes moved under /v1 on the
// studio branch while this exemption was written against "/health" on the api
// branch, and two literals drifting apart is how the exemption silently stops
// matching the route it exempts.
//
