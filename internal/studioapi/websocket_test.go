...
func TestLogsOverWebSocket(t *testing.T) {
	...
	_, resp, br := dialWS(t, addr, "/v1/runs/"+run.ID+"/logs")
...
func TestWebSocketHandshakeRejections(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			fmt.Fprintf(conn, "GET /v1/runs/%s/logs HTTP/1.1\r\nHost: %s\r\n%s\r\n", run.ID, addr, tc.headers)
...
func TestWebSocketTokenViaQueryParameter(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/runs?token=secret", strings.NewReader(`{"command":["echo"]}`))
	req.Host = testHost
...
func TestSSERemainsTheDefaultTransport(t *testing.T) {
	...
	// follow=1, because a *non*-follow request is a JSON document rather than a
	// stream — see handleRunLogs. SSE being the default is a claim about the
	// streaming case: a client that asks to follow and sends no upgrade header
	// gets Server-Sent Events, not a WebSocket.
	rec := doRequest(t, s.Handler(), http.MethodGet, "/v1/runs/"+run.ID+"/logs?follow=1", nil)
