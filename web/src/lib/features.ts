...
export const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    group: "boundary",
    flag: "--runtime",
    body: "Point a run at any OCI runtime the daemon knows: kata-runtime for a microVM with its own kernel, runsc for gVisor's userspace syscall filter. Mounts, hardening, allowlist, caches and secrets all work unchanged on top. Opt-in under dev; under prod it is how a warning becomes a guarantee, and prod refuses outright when the engine reports such a runtime and nothing selected it. Any non-default name counts — gVisor's own installer produces runsc-hostnet, and second-guessing a name you chose deliberately is how a short list turns into a wrong refusal.",
