...
export const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    group: "observability",
    flag: "doctor",
    body: "Asks whether this machine can actually deliver what the profile promises: docker reachable, a syscall filter really applied, a container able to program the egress firewall — tried, not queried, because rootless and userns-remapped daemons cannot — and whether a run here can have a kernel of its own. Under prod the last one has teeth on what it can prove: an engine reporting a microVM runtime that neither the run nor its default uses fails, as does a runtime the engine has not registered and an engine that cannot be asked. One that simply reports none is warned about rather than failed, because its silence is not evidence. --runtime asks about the exact run you are about to make. Non-zero exit, so a scheduler notices before an unattended run does.",
  ...
  {
    title: "Which boundary a run actually got",
    group: "observability",
    body: "The OCI runtime is the one setting that changes the kind of boundary rather than its degree, so a microVM run and a shared-kernel one must not look identical afterwards. The audit line records the runtime a run asked for — omitted when it took the engine default, because naming a runtime nobody chose would be asserting something the tool never asked for — and list and fleet status grow a RUNTIME column the moment any session is on something other than the ordinary shared-kernel runtimes, for every row, since the interesting question is then which runs did not get the boundary you thought you asked for. Read back from the engine rather than remembered from the launch.",
    code: "ID            NAME                   KIND   RUNTIME       STATUS",
    state: "default",
  },
  {
  ...
  },
  {
    ...
    group: "boundary",
    flag: "--runtime",
    body: "Point a run at any OCI runtime the daemon knows: kata-runtime for a microVM with its own kernel, runsc for gVisor's userspace syscall filter. Mounts, hardening, allowlist, caches and secrets all work unchanged on top. Opt-in under dev; under prod it is how a warning becomes a guarantee, and prod refuses outright when the engine reports such a runtime and neither the run nor the engine's default uses it. Setting the engine's own default-runtime satisfies it too. A non-default name we do not recognise is permitted — gVisor's installer produces runsc-hostnet — and reported as not vouched for, since sysbox-runc is also non-default and shares the host kernel.",
  ...
  },
  {
    ...
    group: "workflow",
    flag: "--share",
    body: "Two sandboxes cannot see each other — that is the point, but it leaves no way to hand over an API contract. --share mounts one host directory at /shared in every sandbox that asks for it. Then just say so in the prompt. On native Linux the directory is group-accessible so the container can open it at all: if your primary group is personal nothing else changes, and if it is shared — docker, users, a team group — its members can read /shared too.",
    state: "opt-in",
  },
  {
