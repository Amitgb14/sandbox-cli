...
- Docker — Docker Desktop on macOS/Windows, [Docker Engine on
  Linux](#setting-up-on-linux). [Podman](#using-podman) also works.
...
> Docker](https://docs.docker.com/engine/security/rootless/) avoids it.

Then run an agent from inside a project — never from `$HOME`, which sandbox-cli
refuses to mount:
...
- **Stronger runtimes are opt-in and unvalidated by the host.** `--runtime runsc`
  ...
  the daemon — see [Stronger isolation](#stronger-isolation-microvm--gvisor).
...
straight to `attach`, `logs` or `kill` — see
[Sessions](#sessions-what-is-running-and-how-to-get-at-it) below.
